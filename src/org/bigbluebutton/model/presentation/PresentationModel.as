package org.bigbluebutton.model.presentation
{
  ...
  {    
    ...
    public function replacePresentation(p: Presentation):void {
      ...
      if (oldPres == null) {
        trace("Could not find presentation [{0}] to remove.", [p.id]);
      }
      addPresentation(p);
    }
    ...
    public function getCurrentPresentationName():String {
      for (var i:int = 0; i < _presentations.length; i++) {
        var pres: Presentation = _presentations.getItemAt(i) as Presentation;
        if (pres.current) return pres.fileName;
      }   
    ...
    public function getCurrentPresentation():Presentation {
      ...
      for (var i:int = 0; i < _presentations.length; i++) {
        var pres: Presentation = _presentations.getItemAt(i) as Presentation;
        trace("Is presentation [{0}] current [{1}]?", [pres.fileName, pres.current]);
        if (pres.current) return pres;
      }
      trace("No current presentation.");
      return null;
    }
    ...
    public function getCurrentPage():Slide {
      var pres:Presentation = getCurrentPresentation();
      if (pres != null) {
		return pres.slides[pres.currentSlideNum];
      }
      trace("Could not find current page.");
      return null;
    }
...
//        trace("page id [{0}] ids= [{1},{2}] {3}", [id, presId, pageNum, ids[1]]);
//        var pres:Presentation = getPresentation(presId);
//        var nextPage:int = pageNum + 1;
//        trace("Next page [{0}/{1}]", [presId, nextPage]);
//        if (pres != null) {
//          return pres.getSlideAt(presId + "/" + nextPage);
//        }       
//      }
//      
//      return null;      
...
//        trace("Prev page [{0}/{1}]", [presId, prevPage]);
//        if (pres != null) {
...
//    }
    ...
    public function getPresentation(presId: String):Presentation {
      ...
      trace("Could not find presentation [{0}].", [presId]);
      return null;      
    }
  }
}

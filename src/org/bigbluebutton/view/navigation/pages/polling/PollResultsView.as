...
package org.bigbluebutton.view.navigation.pages.polling
{
	...
	import org.bigbluebutton.view.navigation.pages.common.FocusableImage;
	import mx.resources.ResourceManager;
	
	public class PollResultsView extends VGroup {

		private var _pollGraphic:ResizablePollGraphic;
		private var _topBox:HGroup;
		private var _botBox:HGroup;
		...
		public function PollResultsView(result:Array) {
			...
			publishBtn1.label = ResourceManager.getInstance().getString('resources', 'polling.publishButton.tooltip');
			publishBtn1.addEventListener(MouseEvent.CLICK, handlePublishClick);
			_topBox.addElement(publishBtn1);
			closeBtn1 = new Button();
			closeBtn1.label = ResourceManager.getInstance().getString('resources', 'polling.closeButton.tooltip');
			closeBtn1.addEventListener(MouseEvent.CLICK, handleCloseClick);
			_topBox.addElement(closeBtn1);
			addElement(_topBox);
			...
			publishBtn2.label = ResourceManager.getInstance().getString('resources', 'polling.publishButton.tooltip');
			publishBtn2.addEventListener(MouseEvent.CLICK, handlePublishClick);
			_botBox.addElement(publishBtn2);
			closeBtn2 = new Button();
			closeBtn2.label = ResourceManager.getInstance().getString('resources', 'polling.closeButton.tooltip');
			closeBtn2.addEventListener(MouseEvent.CLICK, handleCloseClick);
			_botBox.addElement(closeBtn2);
			addElement(_botBox);
		...
		private function handlePublishClick(e:MouseEvent):void {
			...
			trace("x1 {0} y1 {1} x2 {2} y2 {3}", [x1, y1, x2, y2]);
			trace("nx1 {0} ny1 {1} nx2 {2} ny2 {3}", [nx1, ny1, nx2, ny2]);
		}
